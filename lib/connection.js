...
Connection.prototype.open = function () {
   ...
   function _start_autoping(session, details, interval){
      self._autoping = setInterval(function(){
            session.publish(`${session._id}/ping`);
      }, interval)
   }

   function _stop_autoping(session, details){
      if(self._autoping)
         clearInterval(self._autoping);
   }  
   ...
   function retry () {
      ...
      self._session.onjoin = function (details) {
         if (self.onopen) {
            try {
               ...
               if(!self._options.autoping_disable)
                  _start_autoping(self._session, details, self._options.autoping_interval ?? 30000);
      ...
      self._transport.onclose = function (evt) {
         ...
         self._autoreconnect_reset_timer();
         _stop_autoping(self._session, details);
